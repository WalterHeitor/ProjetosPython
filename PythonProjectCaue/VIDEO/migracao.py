...
def migrar_dados():
    ...
    while True:
        cursor.execute(f"""
            SELECT nome_arquivo, legenda, titulo, status, status_youtobe,
                   data_criacao, data_postagem, data_post_youtube
            FROM public.michel_bastos_ze_roberto
            ORDER BY id
        ...
        insert_query = """
            INSERT INTO public.lucastylty_pr
            (nome_arquivo, legenda, titulo, status, status_youtobe,
             data_criacao, data_postagem, data_post_youtube)
