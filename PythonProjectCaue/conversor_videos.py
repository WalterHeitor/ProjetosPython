...
def converter_video(input_path, output_path):
    """
    Converte o vídeo para MP4 (HEVC + AAC), usando NVENC para acelerar a codificação de vídeo,
    e CPU para a codificação de áudio (máximo 1080p, 30 fps)
    """
    comando = [
        "ffmpeg", "-y",
        "-i", input_path,
        "-vf", "scale='min(1080,iw)':'min(1920,ih)':force_original_aspect_ratio=decrease",  # Redimensionamento
        "-r", "30",
        "-c:v", "hevc_nvenc",  # Usando o NVENC para codificação de vídeo em HEVC
    ...
    try:
        ...
        # Imprime o erro completo para diagnóstico
        print(f"Erro ao converter o arquivo: {e.stderr}")
        return False
